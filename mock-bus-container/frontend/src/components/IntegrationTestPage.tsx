...
const IntegrationTestPage: React.FC = () => {
  ...
  const getTicket = async () => {
    ...
    setLoading(true);
    try {
      const base64 = btoa(`${selectedApp.name}:${selectedApp.clientSecret}`);
      ...
      message.error('Ошибка получения билета');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card title="Проверка интеграции приложения и каналов" style={{ maxWidth: 700, margin: '32px auto' }}>
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        ...
        {selectedApp && (
          <div>
            <Text>Client Secret: <code>{selectedApp.clientSecret}</code> <CopyOutlined onClick={() => navigator.clipboard.writeText(selectedApp.clientSecret)} /></Text><br/>
            <Text>ID Token: <code>{selectedApp.id_token}</code> <CopyOutlined onClick={() => navigator.clipboard.writeText(selectedApp.id_token)} /></Text>
          </div>
        )}
